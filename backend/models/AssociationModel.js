...
export const getAssociations = (result) => {
    db.query("SELECT * FROM associations ORDER BY association_name ASC", (err, results) => { 
        if(err){
            console.log(err);
            result(err,null)
        } else {
...
export const searchedAssociations = (search, result) => {
    db.query("SELECT * FROM associations WHERE association_name LIKE ? OR association_description LIKE ? OR association_category LIKE ? OR association_representative_name LIKE ? OR association_representative_surname LIKE ? OR association_adress LIKE ? ORDER BY association_name ASC", [`%${search}%`,`%${search}%`,`%${search}%`,`%${search}%`,`%${search}%`,`%${search}%`],
    (err, results) => {
        if(err){
            console.log(err);
            result(err,null)
